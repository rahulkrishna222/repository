...
function blockOccupiedOnPath(selectedPiece, clickedBlock){
	var checkPiece = null,
     	pieceAtBlock = null,
     	i=0;
	var teamOfPieces = json.white;
	do{
		i++;
	if((selectedPiece.col < clickedBlock.col) && selectedPiece.col !== clickedBlock.col - 1){
		checkPiece = selectedPiece.col + 1;
		do{
		
		pieceAtBlock = specialCheck(teamOfPieces, checkPiece, selectedPiece.row);
		if(pieceAtBlock !== null){
			return true;
		}
		checkPiece = checkPiece + 1;
		}while(checkPiece <= (clickedBlock.col - 1));
	}
	else if((selectedPiece.col > clickedBlock.col) && selectedPiece.col !== clickedBlock.col + 1){
		checkPiece = selectedPiece.col - 1;
		do{
	...
	else if((selectedPiece.row > clickedBlock.row) && selectedPiece.row !== clickedBlock.row + 1){
		checkPiece = selectedPiece.row - 1;
		do{
		
		pieceAtBlock = specialCheck(teamOfPieces, selectedPiece.col, checkPiece);
		if(pieceAtBlock !== null){
			return true;
		}
		checkPiece = checkPiece - 1;
		}while(checkPiece >= (clickedBlock.row + 1));
	}
	...
	teamOfPieces = json.black;
	}while(i<2);
	return false; 	
}


function specialCheck(teamOfPieces, checkPiece, rows){
	var curPiece = null,
	iPieceCounter = 0,
	pieceAtBlock = null;
	for (iPieceCounter = 0; iPieceCounter < teamOfPieces.length; iPieceCounter++) {
...
function canPawnMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
	var iRowToMoveTo = 0;
	if(currentTurn === WHITE_TEAM && selectedPiece.row === 6){
		if(clickedBlock.row === 4){
			iRowToMoveTo=selectedPiece.row - 2;
		}
		else{
			iRowToMoveTo=selectedPiece.row - 1;
		}
	}
	else if(currentTurn === BLACK_TEAM && selectedPiece.row === 1){
		if(clickedBlock.row === 3){
			iRowToMoveTo=selectedPiece.row + 2;
		}
		else{
			iRowToMoveTo=selectedPiece.row + 1;
...
function canCastleMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
	var iRowToMoveTo = -1;
	if(selectedPiece.row === clickedBlock.row){
		
			iRowToMoveTo=clickedBlock.row;
		
	}
	else if(selectedPiece.col === clickedBlock.col){
		iRowToMoveTo=clickedBlock.row;
		}
	
bAdjacentEnemy = (clickedBlock.col === selectedPiece.col ||
clickedBlock.row === selectedPiece.row) &&
enemyPiece !== null,
bNextRowEmpty = ((clickedBlock.col === selectedPiece.col || selectedPiece.row === clickedBlock.row) &&
blockOccupied(clickedBlock) === false),
blockOnPath = blockOccupiedOnPath(selectedPiece, clickedBlock);
return clickedBlock.row === iRowToMoveTo && (bNextRowEmpty === true || bAdjacentEnemy === true);
}


function canRookMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
	var iRowToMoveTo = -1;
	if(selectedPiece.row === clickedBlock.row + 1){
	...
	if(selectedPiece.row === clickedBlock.row + 2){
		if(selectedPiece.col === clickedBlock.col - 1){
			iRowToMoveTo=clickedBlock.row;
		}
	}
	if(selectedPiece.row === clickedBlock.row + 2){
...
function canBishopMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
	var iRowToMoveTo = -1;
	if((selectedPiece.row - selectedPiece.col) === (clickedBlock.row - clickedBlock.col)){
		
			iRowToMoveTo=clickedBlock.row;
		
	}
	else if((selectedPiece.row + selectedPiece.col) === (clickedBlock.row + clickedBlock.col)){
...
function canQueenMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
	...
	else if(selectedPiece.row === clickedBlock.row){
...
bAdjacentEnemy = (enemyPiece !== null),
bNextRowEmpty = (blockOccupied(clickedBlock) === false),
blockOnPath = blockOccupiedOnPath(selectedPiece, clickedBlock);
return clickedBlock.row === iRowToMoveTo && (bNextRowEmpty === true || bAdjacentEnemy === true);
}
...
function canKingMoveToBlock(selectedPiece, clickedBlock, enemyPiece) {
	...
	if(selectedPiece.row === clickedBlock.row){
			...
			else if(selectedPiece.col === clickedBlock.col + 1){
				iRowToMoveTo=clickedBlock.row;
			}
	}
	else if(selectedPiece.row === clickedBlock.row - 1){
		if(selectedPiece.col === clickedBlock.col - 1){
			iRowToMoveTo=clickedBlock.row;
		}
		else if(selectedPiece.col === clickedBlock.col + 1){
...
case PIECE_ROUKE:
	...
	bCanMove = canRookMoveToBlock(selectedPiece, clickedBlock, enemyPiece);
